...
const AnalyzeRequestSchema = z.object({
  ...
  url: z.string().url(),
  analysisType: z
    .enum(["summary", "entities", "insights", "keywords"])
    .optional()
    .default("summary"),
});
...
function extractEntities(content: string): string[] {
  ...
  // Apply all patterns
  [...techPatterns, ...companyPatterns, ...locationPatterns].forEach(
    (pattern) => {
...
function generateInsights(
  ...
  const businessTerms = content.match(
    /\b(startup|company|business|revenue|funding|growth|scale)\b/gi,
  );
  ...
  const educationalTerms = content.match(
    /\b(learn|tutorial|guide|how to|example|documentation)\b/gi,
  );
