...
export const PythonCompiler: React.FC<PythonCompilerProps> = ({ onClose, className }) => {
  const [code, setCode] = useState('# Welcome to Python Terminal Compiler!\n# Type your Python code below and click Run\n\nprint("Hello, World!")\n');
  ...
  const loadExample = (example: string) => {
    switch (example) {
      case 'hello':
        ...
        break;
      case 'math':
        setCode('# Math operations\nprint(2 + 3)\nprint(10 * 5)\nprint(100 / 4)\n');
        break;
      case 'loop':
        setCode('# Loop example\nfor i in range(5):\n    print(f"Count: {i}")\n');
        break;
      case 'data':
        setCode('# Data analysis example\nimport pandas as pd\n\n# Create sample data\ndata = {"name": ["Alice", "Bob"], "age": [25, 30]}\nprint("Sample data analysis:")\nprint(data)\n');
        break;
    }
  };
  ...
  return (
    <div className={cn("w-full max-w-4xl mx-auto p-4 space-y-4", className)}>
      <div className="flex items-center justify-between border-b border-green-400/30 pb-2">
        <h2 className="text-xl font-bold text-green-400 font-mono">🐍 Python Terminal Compiler</h2>
        <button
          onClick={onClose}
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="text-green-400 font-mono text-sm">Code Editor:</label>
            <button
              onClick={clearCode}
      ...
      <div className="text-xs text-gray-400 font-mono">
        ...
        <p>⚠️  Note: This is a simulated Python environment for demonstration purposes</p>
      </div>
    </div>
  );
};
